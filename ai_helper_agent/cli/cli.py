...
from ..utils.simple_logo import get_simple_logo, display_cli_header
...
class AIHelperCLI:
    ...
    def show_splash_screen(self):
        ...
        display_cli_header(
            ...
            "Your Autonomous Programming Assistant with Groq models"
        )
    ...
    def _get_system_prompt(self) -> str:
        ...
        return f"""You are AI Helper Agent v2.0.1, an expert AI programming assistant with advanced code generation capabilities created by Meet Solanki, an AIML Student. You specialize in:
        ...
        try:
            ...
            if self.prompt_enhancer:
                try:
                    ...
                    history = user_manager.get_conversation_history(session_id=self.session_id, limit=5)
...
Optimized code:"""
    ...
    def _handle_change_name_command(self, new_name: str) -> str:
        ...
        try:
            ...
            old_conversation_count = len(user_manager.get_conversation_history(session_id=self.session_id, limit=100))
            ...
            if user_manager.change_username(new_name):
                ...
                if migration_choice and old_conversation_count > 0:
                    try:
                        ...
                        old_conversations = user_manager.get_conversation_history(session_id=self.session_id, limit=100)
                        migrated_count = 0
...
Examples:
  ...
  ai-helper --model 2               # Start with specific model
    ...
    parser.add_argument(
        "--version", "-v",
        action="version",
        version="AI Helper Agent CLI v2.0.1"
    )
    ...
    try:
        ...
        if not args.quick:
            ...
            if args.setup:
                ...
                existing_groq_key = api_key_manager.get_api_key('groq')
                if not existing_groq_key:
            ...
            # Quick mode - use existing API key from manager first
