...
class APIKeyManager:
    ...
    def get_api_key(self, provider: str) -> Optional[str]:
        ...
        provider_map = {
            ...
            'gemini': 'GOOGLE_API_KEY',  # GEMINI uses the same key as GOOGLE
        ...
        # For Google/Gemini, try both key names
        if provider.lower() in ['google', 'gemini']:
            return self.keys.get('GOOGLE_API_KEY') or self.keys.get('GEMINI_API_KEY')
    ...
    def set_api_key(self, provider: str, api_key: str, encrypt: bool = True) -> bool:
        ...
        # For Google/Gemini, also remove any old GEMINI_API_KEY entry to avoid confusion
        if provider.lower() in ['google', 'gemini']:
            if 'GEMINI_API_KEY' in self.keys and key_name == 'GOOGLE_API_KEY':
                del self.keys['GEMINI_API_KEY']
    ...
    def list_stored_keys(self) -> Dict[str, bool]:
        ...
        # For Google/Gemini, check both but show as one
        google_key_exists = bool(self.keys.get('GOOGLE_API_KEY') or self.keys.get('GEMINI_API_KEY'))
        stored_keys['GOOGLE_API_KEY'] = google_key_exists
