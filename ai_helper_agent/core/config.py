...
# Import API key manager for enhanced key management
try:
    from ..managers.api_key_manager import api_key_manager
except ImportError:
    # Fallback for when api_key_manager is not available
...
class Config:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        # Override with environment variables and API key manager
        env_config = {
            "api_keys": {
                "groq": api_key_manager.get_api_key('groq') or os.getenv("GROQ_API_KEY"),
                "openai": api_key_manager.get_api_key('openai') or os.getenv("OPENAI_API_KEY"),
                "anthropic": api_key_manager.get_api_key('anthropic') or os.getenv("ANTHROPIC_API_KEY"),
