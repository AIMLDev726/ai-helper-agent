...
from ..utils.mcp_integration import MCPIntegration, MCPSessionManager, MCP_AVAILABLE

try:
    from ..utils.browser_integration import BrowserManager, create_browser_manager, PLAYWRIGHT_AVAILABLE
...
try:
    from ..utils.file_operations import enhanced_file_ops, read_file_enhanced, get_file_capabilities, OCR_AVAILABLE
...
class InteractiveAgent:
    ...
    def __init__(self, llm: Optional[ChatGroq] = None, workspace_path: str = ".", 
        ...
        self.system_prompt = """You are AI Helper Agent v2.0.1, an expert AI programming assistant created by Meet Solanki, an AIML Student, specializing in:
...
Ready to assist with your coding needs!"""
    ...
    def detect_file_type(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            from ..utils.file_operations import detect_file_type
    ...
    def get_file_processing_stats(self) -> Dict[str, Any]:
        ...
        try:
            from ..utils.file_operations import get_file_stats
...
"""
    ...
    def interactive_session(self):
        ...
        print("🤖 AI Helper Agent v2.0.1 - Interactive Session")
        print("=" * 50)
...
Examples:
  ...
  ai-helper mcp-server         # Run as MCP server
    ...
    parser.add_argument(
        "--version", "-v",
        action="version",
        version="AI Helper Agent v2.0.1"
    )
